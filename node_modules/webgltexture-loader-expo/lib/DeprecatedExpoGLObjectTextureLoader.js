...
const available = !!(_core.NativeModulesProxy.ExponentGLObjectManager && _core.NativeModulesProxy.ExponentGLObjectManager.createObjectAsync);
let warned = false;
...
class ExpoGLObjectTextureLoader extends _webgltextureLoader.WebGLTextureLoaderAsyncHashCache {
  ...
  loadNoCache(config) {
    ...
    const promise = _core.NativeModulesProxy.ExponentGLObjectManager.createObjectAsync({
      exglCtxId,
      texture: config
    }).then(({
      exglObjId
